...
import { useRoqPlatformUserProfiles, useServiceCount, usePipelineCount, useAppointmentCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const pipelineStat = usePipelineCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Pipelines',
      result: pipelineStat?.data,
      isLoading: pipelineStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Appointments',
