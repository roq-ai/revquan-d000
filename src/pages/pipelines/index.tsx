...
import { useRoqClient, usePipelineFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<PipelineInterface, unknown>;
...
export function PipelineListPage(props: PipelineListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['progress_status.contains'],
          ...params.filters,
        },
        'pipeline',
      ),
  ...
  const { data, error, isLoading, mutate } = usePipelineFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.pipeline.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.expected_close_date ? format(record?.expected_close_date, 'dd-MM-yyyy') : '',
    },
    { id: 'progress_status', header: 'Progress Status', accessorKey: 'progress_status' },
    !hideActions
      ? {
          id: 'actions',
          header: '',
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/pipelines/view/${record.id}`} passHref legacyBehavior>
                <Button
